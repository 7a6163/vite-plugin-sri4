...
The mutation score is **81.19%**, with a break threshold of 80. It runs weekly in CI rather than on every push — a full run re-executes the suite once per mutant, ~4 minutes for 707 mutants, which is too slow to gate a commit for a signal that moves this slowly. Run it on demand from the Actions tab, or locally with the command above; the HTML report is uploaded as an artifact either way. The remaining survivors are mostly equivalent mutants — warning message wording, cache clearing that has no observable effect, and `typeof source === 'string' ? source : Buffer.from(source)`, whose two branches hash identically. Killing those would mean asserting log text verbatim, which costs more than it protects.
