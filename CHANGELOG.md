...
- **The Vite version floor in the "no import-analysis plugin" error was wrong.** It promised "Requires Vite 6.0.0 or higher" while `peerDependencies` has been `^6.4.0` and the CI compatibility matrix only covers 6.4 — a floor four minors below anything actually claimed or tested. Anyone who hit that error on 6.2 was told their Vite was supported.

### Internal

- **The source is now TypeScript.** `src/` is six `.ts` modules plus a `types.ts` holding the public option types, and `types/index.d.ts` is generated by `tsc` rather than hand-maintained beside a JS implementation. It had not actually drifted in five releases — the one near-miss was an `@default` comment, which TypeScript does not check either — so the real gain is type-checked interaction with Rollup's plugin API, not drift protection.

  The published type surface is unchanged and held there by `test/types/contract.ts`, which asserts against the *generated* declarations rather than the source: all eight `SriOptions` fields via `Required<SriOptions>` so a removed field fails as an excess property and an added one as a missing property, the three SRI algorithms, both `crossorigin` values, all five log levels, and that both exports are assignable to Vite's `Plugin`. Widening any of them fails the build.

  Behaviour is unchanged, and the test suite is the evidence: not one line of it was rewritten for the migration, because vitest resolves `../src/cache.js` to `src/cache.ts`. The only test change here is the assertion below, which closes a gap the migration exposed rather than one it needed accommodating.

  `types/` now ships seven declaration files instead of one. `index.d.ts` re-exports from `types.d.ts`; the rest are internal modules nothing in the public graph reaches.

- **`typescript` is pinned to `^5`.** The `typescript@7` package no longer exposes the classic compiler API — `ModuleKind`, `ScriptTarget`, `createProgram` and `sys` are all `undefined` — and `@rollup/plugin-typescript` needs it. The `tsc` CLI itself works on 7, so this is a plugin-ecosystem constraint rather than a language one.

- **Runtime validation still accepts wide input.** `validateOptions` and `Logger`'s level fallback take `string`, not the narrowed published types, because a plain JS `vite.config.js` gets no type checking at all and those throws are the only thing between `hashAlgorithm: 'md5'` and a resource the browser blocks with no build-time error. `validateOptions` returns its values narrowed, so the validation is what produces the types the rest of the plugin relies on.

- **CI typechecks and builds on every PR.** Neither ran before, so a type error or a broken bundle would only have surfaced when a release tag was pushed. The build job also runs `git diff --exit-code types/`: the generated declarations are committed so every PR diff shows changes to the published type surface, and without that check the committed copy drifts and the diff stops meaning anything.

### Testing

- **`manifest: true` is now pinned to *not* inject an import map.** Both options read the same computed hashes, so an injection that is not gated on `importmap` would ship an import map, carrying real integrity entries, to everyone who only asked for a manifest. Nothing asserted this. It had been covered by accident: `hashes` was `null` when neither option was set, so an ungated injection died on `Object.entries(null)` throwing — a crash standing in for a test. The nullable is gone and the assertion is real.

- Mutation score is 81.13%, break threshold 80. Survivors are unchanged at 133 from the pre-migration source and identical in every file, so nothing got weaker; the difference is four mutants Stryker no longer generates because `SUPPORTED_HASH_ALGORITHMS` is an `as const` array. Paid deliberately — deriving `SriHashAlgorithm` from that single list is what makes the runtime check and the published type unable to disagree, and the contract test pins the three literals at the type level instead.

### Documentation

- **The plugin-reposition comment records a Vite 6 measurement.** It cited only Vite 8.2.2, which left open whether the reposition is a Vite 8 quirk that Vite 6 does not need. Measured on 6.4.3 as well: a plain `enforce: 'post'` plugin lands immediately before the analysis plugin and sees `__VITE_PRELOAD__` unsubstituted in its own `generateBundle`, while the emitted file carries none. The failure is identical on every supported major.
